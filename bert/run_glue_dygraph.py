...
def do_train(args):
    ...
    with LogWriter(logdir="./log/low_dygraph_"+str(mode)) as writer:
        for epoch in range(args.num_train_epochs):
            for step, batch in enumerate(train_data_loader):
                ...
                if not args.use_amp:
                    ...
                    with paddle.amp.auto_cast(custom_white_list=['layer_norm', 'softmax', 'gelu'], custom_black_list=['lookup_table', 'lookup_table_v2']):
                        logits = model(input_ids, segment_ids)
                        loss = loss_fct(logits, labels)
