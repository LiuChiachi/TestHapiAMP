...
def do_train(args):
    mode = "O1" if args.use_amp else "O0"
    dynamic = True
    ...
    inputs = [InputSpec([None, None], "int64", "src"),
            InputSpec([None, None], "int64", "trg")]
    labels = [InputSpec([None, None, 1], "int64", "label")]
    model = paddle.hapi.Model(transformer, inputs, labels)
    amp_configs = {'level': mode, "init_loss_scaling": 128.0, "custom_white_list":{'softmax', 'layer_norm'},
                    "custom_black_list": {'lookup_table_v2'}} if not dynamic else {'level': mode, "init_loss_scaling": 128.0}
    
    model.prepare(optimizer=optimizer, loss=criterion, amp_configs=amp_configs)
    model.fit(train_loader, epochs=1, log_freq=100,callbacks=[paddle.callbacks.VisualDL(log_dir="./log/"+"hapi_"+str(mode))])
